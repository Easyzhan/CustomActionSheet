...
CGFloat const ZActionSheetSectionHeight = 10; ///< 分区间距
@interface ZActionSheet()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, copy) NSString *title;
@property (nonatomic, copy) NSString *cancelTitle;
@property (nonatomic, copy) NSArray<ZActionSheetItem *> *items;
@property (nonatomic, copy) ZActionSheetHandler selectedHandler;
...
static NSString * const ZActionSheetCellIdentifier = @"ZActionSheetCellIdentifier";
...
//    NSLog(@"-- ZActionSheet did dealloc -- [%p] -- [%p] -- [%p]", _window, _backView, _tableView);
}
...
- (instancetype)initWithTitle:(NSString *)title delegate:(id<ZActionSheetDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle highlightedButtonTitle:(NSString *)highlightedButtonTitle otherButtonTitles:(NSArray<NSString *> *)otherButtonTitles {
    ...
    for (NSString *otherTitle in otherButtonTitles) {
        if (otherTitle && otherTitle.length > 0) {
            [titleItems addObject:ZActionSheetTitleItemMake(ZActionSheetTypeNormal, otherTitle)];
        }
    }
    // 高亮按钮, 高亮按钮放在最下面.
    if (highlightedButtonTitle && highlightedButtonTitle.length > 0) {
        [titleItems addObject:ZActionSheetTitleItemMake(ZActionSheetTypeHighlighted, highlightedButtonTitle)];
    }
...
// 默认设置
- (void)baseSetting {
    self.backgroundColor = ZColorWithString(ZActionSheetBackColor);
    self.translatesAutoresizingMaskIntoConstraints = NO; // 允许约束
    
    _contentAlignment = ZContentAlignmentCenter; // 默认样式为居中
...
- (CGFloat)heightForHeaderView {
    CGFloat labelHeight = [_titleLabel.attributedText boundingRectWithSize:CGSizeMake(CGRectGetWidth(self.window.frame)-ZActionSheetDefaultMargin*2, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading context:nil].size.height;
    CGFloat headerHeight = ceil(labelHeight)+ZActionSheetDefaultMargin*2;
...
- (CGFloat)contentHeight {
    ...
    CGFloat rowHeightSum = (_items.count+1)*ZActionSheetRowHeight+ZActionSheetSectionHeight;
...
- (void)fixContentHeight {
    CGFloat contentMaxHeight = CGRectGetHeight(self.window.frame)*ZActionSheetContentMaxScale;
...
- (void)updateTitleAttributeText {
    ...
    UIColor *titleTextColor = ZColorWithString(ZActionSheetTitleColor);
    CGFloat  lineSpacing = ZActionSheetTitleLineSpacing;
    CGFloat  kernSpacing = ZActionSheetTitleKernSpacing;
...
- (UITableView *)tableView {
    ...
    [_tableView registerClass:[ZActionSheetCell class] forCellReuseIdentifier:ZActionSheetCellIdentifier];
...
- (UIView *)headerView {
    ...
    headerView.backgroundColor = ZColorWithString(ZActionSheetRowNormalColor);
    ...
    // 标题内容边距 (ps: 要修改这个边距不要在这里修改这个labelMargin, 要到配置类中修改 ZActionSheetDefaultMargin, 不然可能出现界面适配错乱).
    CGFloat labelMargin = ZActionSheetDefaultMargin;
    // 计算内容高度
...
- (NSInteger)numberOZectionsInTableView:(UITableView *)tableView {
    return 2;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return ZActionSheetRowHeight;
}
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    return (section == 1)?ZActionSheetSectionHeight:CGFLOAT_MIN;
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZActionSheetCell *cell = [tableView dequeueReusableCellWithIdentifier:ZActionSheetCellIdentifier];
    return cell;
}
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    ZActionSheetCell *sheetCell = (ZActionSheetCell *)cell;
    if (indexPath.section == 0) {
        ...
        // 默认取消的单元格没有附带icon
        ZActionSheetItem *cancelItem = ZActionSheetTitleItemMake(ZActionSheetTypeNormal, _cancelTitle);
        // 如果其它单元格中附带icon的话则添加上默认的取消icon.
        for (ZActionSheetItem *item in _items) {
            if (item.image) {
                cancelItem = ZActionSheetTitleWithImageItemMake(ZActionSheetTypeNormal, [UIImage imageNamed:@"ZActionSheet_cancel"], _cancelTitle);
                break;
            }
        }
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1*NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [self hideWithCompletion:^{
            if (indexPath.section == 0) {
                ...
                if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(ZActionSheet:selectedIndex:)]) {
                    [weakSelf.delegate ZActionSheet:self selectedIndex:indexPath.row];
                }
            }
        }];
    });
